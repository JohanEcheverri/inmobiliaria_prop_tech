...
function AdminTables({ seccion, data = [], onEdit, onDelete, onAssignSpecialty }) {

    // 1. RENDER PARA LA SECCIÓN DE INMUEBLES
    if (seccion === 'inmuebles') {
        return (
            <table className="domustech-admin-table">
                ...
                <tbody>
                {data.length === 0 ? (
                    <tr><td colSpan="8" className="table-empty-row">No hay inmuebles registrados.</td></tr>
                ) : (
                    data.map((inm) => (
                        <tr key={inm.codigo}>
                            <td><strong>{inm.codigo}</strong></td>
                            <td><span className={`badge-type ${inm.finalidad?.toLowerCase()}`}>{inm.tipoInmueble} ({inm.finalidad})</span></td>
    ...
    // 2. RENDER PARA LAS SECCIONES DE PERSONAS (CLIENTES / ASESORES)
    return (
        <table className="domustech-admin-table">
            <thead>
            <tr>
                ...
                <th>Teléfono</th>
                {seccion === 'asesores' && <th>Especialidad / Zona</th>}
                <th>Acciones</th>
            </tr>
            </thead>
            <tbody>
            {data.length === 0 ? (
                <tr><td colSpan={seccion === 'asesores' ? 7 : 6} className="table-empty-row">No hay {seccion} registrados.</td></tr>
            ) : (
                data.map((item) => (
                    <tr key={item.identificacion}>
                        ...
                        <td>
                            {seccion === 'asesores' && (
                                <button className="btn-action-svg-assign" onClick={() => onAssignSpecialty(item)} title="Asignar Especialidad e Inmuebles">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="table-icon-svg"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"></path></svg>
                                </button>
                            )}
                            <button className="btn-action-svg-edit" onClick={() => onEdit(item)} title="Editar">
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="table-icon-svg"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
                            </button>
                            <button className="btn-action-svg-delete" onClick={() => onDelete(item.identificacion, seccion)} title="Eliminar">
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="table-icon-svg"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path><line x1="10" y1="11" x2="10" y2="17"></line><line x1="14" y1="11" x2="14" y2="17"></line></svg>
                            </button>
                        </td>
                    </tr>
